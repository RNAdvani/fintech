...
import { CurrencyType, ExpenseType } from "@/types"; // Ensure this path is correct
...
import { Expense } from "@my-org/types";

interface ExpenseCardProps {
  key: string; // Ensure key is included if needed
  expense: ExpenseType; // Adjust according to your types
  currency: CurrencyType;
  onExpenseUpdated?: (updatedExpense: ExpenseType) => void;
    ...
    <Card className="bg-background relative">
      <CardContent className="grid grid-cols-4 sm:grid-cols-5 lg:grid-cols-7 items-center gap-y-2 px-4 sm:px-6 py-3">
        ...
        <span className="col-span-2 sm:col-span-3 break-words">
          {expense.description}{" "}
          {expense.dueType && expense.dueType === "payable" as string && (
